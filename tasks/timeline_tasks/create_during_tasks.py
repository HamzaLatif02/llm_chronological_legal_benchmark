...
# Generate valid legislation pairs for comparison
valid_pairs = []
...
for task_id, (lawA, lawB) in enumerate(sampled_pairs, start=1):
    ...
    # Ground truth condition: A starts after B starts, and ends before B ends
    is_during = startB < startA < endA < endB
    during_text = "yes" if is_during else "no"

    task = {
        "task_id": f"DU{task_id:03}",
        "task_type": "during_task",
        "prompt": f"Is '{titleA}' during '{titleB}'?",
        "prompt_with_dates": (
            f"Is '{titleA}' (from {startA_raw} to {endA_raw}) during "
...
with open("during_tasks_with_answers.json", "w", encoding="utf-8") as f:
    json.dump(tasks_with_answers, f, indent=2, ensure_ascii=False)

# Save tasks without answers (test set) in a JSON file
with open("during_tasks_without_answers.json", "w", encoding="utf-8") as f:
    json.dump(tasks_without_answers, f, indent=2, ensure_ascii=False)

print(f"Generated {len(tasks_with_answers)} 'during' tasks with and without date prompts.")
