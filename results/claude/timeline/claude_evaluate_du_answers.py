...
claude_answers = [ans for ans in claude_answers if ans.get("task_id", "").startswith("DU")]
...
for ans in claude_answers:
    ...
    claude_main = find_yes_no(ans.get("answer", ""))
    claude_dates = find_yes_no(ans.get("answer_with_dates", ""))

    result = {
        "task_id": task_id,
        "expected": expected,
        "claude_main_answer": claude_main,
        "claude_with_dates_answer": claude_dates,
        "correct_main": claude_main == expected,
        "correct_with_dates": claude_dates == expected,
        "claude_main_snippet": ans.get("answer", "")[:200],
        "claude_with_dates_snippet": ans.get("answer_with_dates", "")[:200]
    }
...
print("Results saved to 'claude_du_responses.json'")

# Save results in JSON format
with open("claude_du_responses.json", "w", encoding="utf-8") as f:
