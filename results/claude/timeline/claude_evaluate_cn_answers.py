...
claude_answers = [ans for ans in claude_answers if ans.get("task_id", "").startswith("CN")]
...
for claude in claude_answers:
    ...
    claude_main = find_yes_no(claude.get("answer", ""))
    claude_dates = find_yes_no(claude.get("answer_with_dates", ""))

    main_correct = claude_main == expected
    dates_correct = claude_dates == expected

    results.append({
        "task_id": task_id,
        "expected": expected,
        "found_in_answer": claude_main,
        "found_in_answer_with_dates": claude_dates,
        "correct_main": main_correct,
        "correct_with_dates": dates_correct,
        "claude_text_snippet": claude.get("answer", "")[:150]
    })
...
print("Results saved to 'claude_cn_responses.json'")

# Save results in JSON format
with open("claude_cn_responses.json", "w", encoding="utf-8") as f:
