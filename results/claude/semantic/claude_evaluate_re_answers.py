...
claude_data = [x for x in claude_data if x.get("task_id", "").startswith("RE")]
...
for item in claude_data:
    ...
    claude_recitals = extract_recital_numbers(claude_text)

    # Check for correctness
    correct = sorted(gt_recitals) == sorted(claude_recitals)
    missing = sorted(set(gt_recitals) - set(claude_recitals))
    extra = sorted(set(claude_recitals) - set(gt_recitals))

    results.append({
        "task_id": task_id,
        "correct": correct,
        "claude_recitals": claude_recitals,
...
print("Results saved to 'claude_re_responses.json'")

# Save results in JSON format
with open("claude_re_responses.json", "w", encoding="utf-8") as f:
