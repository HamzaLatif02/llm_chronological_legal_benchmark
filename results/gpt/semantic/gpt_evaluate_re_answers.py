...
gpt_data = [x for x in gpt_data if x.get("task_id", "").startswith("RE")]
...
for item in gpt_data:
    ...
    gpt_recitals = extract_recital_numbers(gpt_text)

    # Check for correctness
    correct = sorted(gt_recitals) == sorted(gpt_recitals)
    missing = sorted(set(gt_recitals) - set(gpt_recitals))
    extra = sorted(set(gpt_recitals) - set(gt_recitals))

    results.append({
        "task_id": task_id,
        "correct": correct,
        "gpt_recitals": gpt_recitals,
...
print("Results saved to 'gpt_re_responses.json'")

# Save results in JSON format
with open("gpt_re_responses.json", "w", encoding="utf-8") as f:
