...
gpt_data = [x for x in gpt_data if x.get("task_id", "").startswith("RC")]
...
for item in gpt_data:
    ...
    for title in expected_titles:
        if normalise(title) in gpt_norm:
            found.append(title)
        else:
...
print("Results saved to 'gpt_rc_responses.json'")

# Save results in JSON format
with open("gpt_rc_responses.json", "w", encoding="utf-8") as f:
