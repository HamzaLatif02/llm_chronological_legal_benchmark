...
gpt_answers = [ans for ans in gpt_answers if ans.get("task_id", "").startswith("OL")]
...
for ans in gpt_answers:
    ...
    gpt_main_raw = ans.get("answer", "")
    gpt_dates_raw = ans.get("answer_with_dates", "")

    gpt_main = infer_overlap_answer(gpt_main_raw)
    gpt_dates = infer_overlap_answer(gpt_dates_raw)

    result = {
        "task_id": task_id,
        "expected_overlap": expected,
        "gpt_answer_inferred": gpt_main,
        "gpt_with_dates_inferred": gpt_dates,
        "correct_main": gpt_main == expected,
        "correct_with_dates": gpt_dates == expected,
        "gpt_main_snippet": gpt_main_raw[:250],
        "gpt_with_dates_snippet": gpt_dates_raw[:250]
    }
...
print("Results saved to 'gpt_ol_responses.json'")

# Save results in JSON format
with open("gpt_ol_responses.json", "w", encoding="utf-8") as f:
