...
gpt_answers = [ans for ans in gpt_answers if ans.get("task_id", "").startswith("DU")]
...
for ans in gpt_answers:
    ...
    gpt_main = find_yes_no(ans.get("answer", ""))
    gpt_dates = find_yes_no(ans.get("answer_with_dates", ""))

    result = {
        "task_id": task_id,
        "expected": expected,
        "gpt_main_answer": gpt_main,
        "gpt_with_dates_answer": gpt_dates,
        "correct_main": gpt_main == expected,
        "correct_with_dates": gpt_dates == expected,
        "gpt_main_snippet": ans.get("answer", "")[:200],
        "gpt_with_dates_snippet": ans.get("answer_with_dates", "")[:200]
    }
...
print("Results saved to 'gpt_du_responses.json'")

# Save results in JSON format
with open("gpt_du_responses.json", "w", encoding="utf-8") as f:
