...
gpt_answers = [ans for ans in gpt_answers if ans.get("task_id", "").startswith("CO")]
...
# Check if older and newer assignments are correct in GPT answer
...
for ans in gpt_answers:
    ...
    main_correct = check_comparison(gpt_main, older, newer)
    dates_correct = check_comparison(gpt_dates, older, newer)
    ...
    results.append({
        ...
        "gpt_main_snippet": gpt_main[:300],
        "gpt_with_dates_snippet": gpt_dates[:300]
    })
...
print("Results saved to 'gpt_co_responses.json'")

# Save results in JSON format
with open("gpt_co_responses.json", "w", encoding="utf-8") as f:
